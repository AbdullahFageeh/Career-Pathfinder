...
const sampleResume: TailoredResume = {
  ...
  outputPath: "/tmp/job-site-manager.html",
...
const sampleQueueJob: QueueJob = {
  ...
  scheduledFor: "2026-06-09T13:02:00.000Z",
  createdAt: "2026-06-09T13:02:00.000Z",
  updatedAt: "2026-06-09T13:02:00.000Z"
};
...
test("sqlite storage persists pipeline artifacts and queue jobs across instances", async (t) => {
  ...
  assert.equal(Object.keys(snapshot.queueJobs).length, 1);
  ...
  assert.deepEqual(await secondStorage.getQueueJob(sampleQueueJob.id), sampleQueueJob);
  assert.deepEqual(
    await secondStorage.getQueueJobByIdempotencyKey(sampleQueueJob.idempotencyKey),
    sampleQueueJob
  );
});
...
test("sqlite storage claims the oldest due queue job and records the lease owner", async (t) => {
  ...
  const claimedQueueJob = await storage.claimNextQueueJob({
    workerId: "worker:test",
    now: "2026-06-09T13:10:00.000Z"
  });

  assert.equal(claimedQueueJob?.id, sampleQueueJob.id);
  assert.equal(claimedQueueJob?.state, "processing");
  assert.equal(claimedQueueJob?.attempts, 1);
  assert.equal(claimedQueueJob?.workerId, "worker:test");
  assert.ok(claimedQueueJob?.leaseExpiresAt);
});
